...
export default function Guardian() {
  ...
  return (
    <div className="flex flex-col h-full mt-4 animate-fade-in">
      <h2 className="text-2xl font-bold text-[var(--dark)] mb-6">{t('minor.guardianTitle')}</h2>
      
      <div className="flex flex-col gap-4 flex-1">
        <div>
          <label className="label">{t('minor.relationship')}</label>
          <select value={relationship} onChange={e => setRel(e.target.value)} className="input-field bg-white">
            <option value="FATHER">{t('minor.father')}</option>
            <option value="MOTHER">{t('minor.mother')}</option>
            <option value="LEGAL_GUARDIAN">{t('minor.legalGuardian')}</option>
          </select>
        </div>
        
        <div>
          <label className="label">{t('minor.guardianFullName')}</label>
          <input type="text" value={fullName} onChange={e => setFullName(e.target.value)} className="input-field" placeholder="As per PAN" />
        </div>

        <div>
          <label className="label">{t('minor.guardianPan')}</label>
        ...
        <div>
          <label className="label">{t('minor.guardianMobile')}</label>
